# GAME CONFIG
TITLE = 'Zombie Sunday'
WINDOW_SIZE = (1280, 600)
FPS = 60
RES_DIR = 'res'
FONT = 'res/fonts/ProggySquare.ttf'

# LEVEL POSITIONS
LOGO_POS = ((WINDOW_SIZE[0] / 2) - 338, 30)
BUTTON_START_POS = ((WINDOW_SIZE[0] / 2) - 165, 220)

SUN_POS = (WINDOW_SIZE[0] - 250, 20)
BACKGROUND_1_POS = (0, WINDOW_SIZE[1] - 245)
BACKGROUND_2_POS = (0, WINDOW_SIZE[1] - 192)
CLOUD_1_POS = (WINDOW_SIZE[0] - 100, 10)
CLOUD_2_POS = (WINDOW_SIZE[0] / 2, 200)
CLOUD_3_POS = (0, 100)
GROUND_POS = (0, WINDOW_SIZE[1] - 96)

PLAYER_POS = (10, WINDOW_SIZE[1] - 300)
HOME_POS = (5, WINDOW_SIZE[1] - 525)
ZOMBIE_POS = (0, WINDOW_SIZE[1] - 246)

# LEVEL CONFIG
LEVEL_SIZE = 10
GROUND_WIDTH = 1600
GROUND_HEIGHT = 96
TITLE_SCROLL_SPEED = 100

BUILDING_RANGE = [15, 25, 100] # [Min, max, multiplier]. randint(min, max) * multiplier
ZOMBIE_MAX = 10
ZOMBIE_SPAWN_TIME = 2 # In seconds

ZOMBIE_MAX_MOD = 1 # ZOMBIE_MAX += ZOMBIE_MAX_MOD * level.stage
ZOMBIE_SPAWN_TIME_MOD = .98 # ZOMBIE_SPAWN_TIME *= pow(ZOMBIE_SPAWN_TIME_MOD, level.stage)
ZOMBIE_HSPEED_MOD = 5 # ZOMBIE_HSPEED += ZOMBIE_HSPEED_MOD * level.stage
ZOMBIE_DAMAGE_MOD = 5 # ZOMBIE_DAMAGE += ZOMBIE_DAMAGE_MOD * level.stage
ZOMBIE_HEALTH_MOD = 10 # ZOMBIE_HEALTH += ZOMBIE_HEALTH_MOD * level.stage
ZOMBIE_ATTACK_RATE_MOD = .5 # ZOMBIE_ATTACK_RATE -= ZOMBIE_ATTACK_RATE_MOD * level.stage
ZOMBIE_SCORE_MOD = 10 # ZOMBIE_SCORE += ZOMBIE_SCORE_MOD * level.stage
BUILDING_DISTANCE_MOD = 20 # BUILDING_RANGE += BUILDING_DISTANCE_MOD * level.stage

# ENTITY CONFIG
ENTITY_GRAVITY = 20
ENTITY_VSPEED = 200
ENTITY_HSPEED = 200
ENTITY_HEALTH = 100

PLAYER_AMMO = 500
PLAYER_HEALTH = 100
PLAYER_HSPEED = 400

ZOMBIE_HEALTH = 100
ZOMBIE_HSPEED = 75
ZOMBIE_DAMAGE = 10
ZOMBIE_ATTACK_RATE = 1
ZOMBIE_SCORE = 100

# WEAPON CONFIG
DEFAULT_RATE = 1
DEFAULT_AMMO_CONSUMPTION = 1
DEFAULT_DURATION = 1 # In seconds
DEFAULT_HSPEED = 600
DEFAULT_DAMAGE = 10
DEFAULT_ADD_PLAYER_SPEED = True

SPEED_RATE = 1
SPEED_AMMO_CONSUMPTION = 1
SPEED_DURATION = 1 # In seconds
SPEED_HSPEED = 1000
SPEED_DAMAGE = 8
SPEED_ADD_PLAYER_SPEED = True

SPEEDSPEED_RATE = 1
SPEEDSPEED_AMMO_CONSUMPTION = 1
SPEEDSPEED_DURATION = 1 # In seconds
SPEEDSPEED_HSPEED = 1800
SPEEDSPEED_DAMAGE = 5
SPEEDSPEED_ADD_PLAYER_SPEED = True

SPEEDFIRE_RATE = 1
SPEEDFIRE_AMMO_CONSUMPTION = 1
SPEEDFIRE_DURATION = 1 # In seconds
SPEEDFIRE_HSPEED = 700
SPEEDFIRE_DAMAGE = 6
SPEEDFIRE_ADD_PLAYER_SPEED = True
SPEEDFIRE_FIRE_DAMAGE = 10
SPEEDFIRE_FIRE_DURATION = 3 # In seconds
SPEEDFIRE_FIRE_RATE = .5

SPEEDLOB_RATE = 1
SPEEDLOB_AMMO_CONSUMPTION = 1
SPEEDLOB_DURATION = 1 # In seconds
SPEEDLOB_HSPEED = 700
SPEEDLOB_VSPEED = -400
SPEEDLOB_GRAVITY = 50
SPEEDLOB_ADD_PLAYER_SPEED = True
SPEEDLOB_EXPLOSION_DURATION = 1 # In seconds
SPEEDLOB_EXPLOSION_DAMAGE = 5

SPEEDFORCE_RATE = 1
SPEEDFORCE_AMMO_CONSUMPTION = 1
SPEEDFORCE_DURATION = .7 # In seconds
SPEEDFORCE_HSPEED = 1000
SPEEDFORCE_DAMAGE = 0
SPEEDFORCE_ADD_PLAYER_SPEED = True
SPEEDFORCE_FORCE_HSPEED = 100 # zombie.hspeed set to: HSPEED + FORCE_HSPEED
SPEEDFORCE_FORCE_VSPEED = -220

FIRE_RATE = 1
FIRE_AMMO_CONSUMPTION = 1
FIRE_DURATION = .7 # In seconds
FIRE_HSPEED = 300
FIRE_DAMAGE = 10
FIRE_ADD_PLAYER_SPEED = True
FIRE_FIRE_DAMAGE = 10
FIRE_FIRE_DURATION = 3 # In seconds
FIRE_FIRE_RATE = .5

FIREFIRE_RATE = 1
FIREFIRE_AMMO_CONSUMPTION = 1
FIREFIRE_DAMAGE = 1
FIREFIRE_FIRE_DAMAGE = 10
FIREFIRE_FIRE_DURATION = 3 # In seconds
FIREFIRE_FIRE_RATE = .5

FIRELOB_RATE = 1
FIRELOB_AMMO_CONSUMPTION = 1
FIRELOB_DURATION = 1 # In seconds
FIRELOB_HSPEED = 500
FIRELOB_VSPEED = -500
FIRELOB_GRAVITY = 20
FIRELOB_ADD_PLAYER_SPEED = True
FIRELOB_EXPLOSION_DURATION = 1 # In seconds
FIRELOB_EXPLOSION_DAMAGE = 5
FIRELOB_FIRE_DAMAGE = 10
FIRELOB_FIRE_DURATION = 3 # In seconds
FIRELOB_FIRE_RATE = .5

FIREFORCE_RATE = 1
FIREFORCE_AMMO_CONSUMPTION = 1
FIREFORCE_DURATION = .7 # In seconds
FIREFORCE_HSPEED = 500
FIREFORCE_DAMAGE = 0
FIREFORCE_ADD_PLAYER_SPEED = True
FIREFORCE_FORCE_HSPEED = 100 # zombie.hspeed set to: HSPEED + FORCE_HSPEED
FIREFORCE_FORCE_VSPEED = -220
FIREFORCE_FIRE_DAMAGE = 10
FIREFORCE_FIRE_DURATION = 3 # In seconds
FIREFORCE_FIRE_RATE = .5

LOB_RATE = 1
LOB_AMMO_CONSUMPTION = 1
LOB_DURATION = 1 # In seconds
LOB_HSPEED = 500
LOB_VSPEED = -500
LOB_GRAVITY = 20
LOB_ADD_PLAYER_SPEED = True
LOB_EXPLOSION_DURATION = 1 # In seconds
LOB_EXPLOSION_DAMAGE = 5

LOBLOB_RATE = 1
LOBLOB_AMMO_CONSUMPTION = 1
LOBLOB_DURATION = 1 # In seconds
LOBLOB_HSPEED = 500
LOBLOB_VSPEED = -500
LOBLOB_GRAVITY = 20
LOBLOB_ADD_PLAYER_SPEED = True
LOBLOB_EXPLOSION_DURATION = 1 # In seconds
LOBLOB_EXPLOSION_DAMAGE = 5

LOBFORCE_RATE = 1
LOBFORCE_AMMO_CONSUMPTION = 1
LOBFORCE_DURATION = 1 # In seconds
LOBFORCE_HSPEED = 500
LOBFORCE_VSPEED = -500
LOBFORCE_GRAVITY = 20
LOBFORCE_ADD_PLAYER_SPEED = True
LOBFORCE_FORCE_HSPEED = 600 # zombie.hspeed set to FORCE_HSPEED
LOBFORCE_FORCE_VSPEED = -220
LOBFORCE_EXPLOSION_DURATION = 1 # In seconds
LOBFORCE_EXPLOSION_DAMAGE = 5

FORCE_RATE = 1
FORCE_AMMO_CONSUMPTION = 1
FORCE_DURATION = .7 # In seconds
FORCE_HSPEED = 500
FORCE_DAMAGE = 0
FORCE_ADD_PLAYER_SPEED = True
FORCE_FORCE_HSPEED = 100 # zombie.hspeed set to: HSPEED + FORCE_HSPEED
FORCE_FORCE_VSPEED = -220

FORCEFORCE_RATE = 1
FORCEFORCE_AMMO_CONSUMPTION = 1
FORCEFORCE_DURATION = .8 # In seconds
FORCEFORCE_HSPEED = 500
FORCEFORCE_DAMAGE = 0
FORCEFORCE_ADD_PLAYER_SPEED = True
FORCEFORCE_FORCE_HSPEED = 300 # zombie.hspeed set to: HSPEED + FORCE_HSPEED
FORCEFORCE_FORCE_VSPEED = -330