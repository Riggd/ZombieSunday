...
class UI_Group:
	def __init__(self,somber,level,z):
		self.somber = somber
		self.level = level
		self.z = z
	...
	def create_element(self,sprite,x=0,y=0):
		_element = UI_Element(self.somber,self.level,sprite,x=x,y=y,z=self.z)
		
		self.elements.append(_element)

class UI_Element(somber_engine.Active):
	def __init__(self,somber,level,sprite,x=0,y=0,z=0):
		somber_engine.Active.__init__(self,sprite,somber=somber,pos=(x,y))
		
		level.add_object(self,z=z)